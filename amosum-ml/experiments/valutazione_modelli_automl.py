...
# ==========================================
# 0. WRAPPER FOLD-R++ (INCLUSO PER IL DESERIALIZE DI JOBLIB SE NECESSARIO)
# ==========================================
try:
...
# Modelli pre-addestrati tramite AutoML
...
def validazione_finale(dominio, model_path, test_path, original_paths):
    print(f"\n{'='*80}\n🚀 VALIDAZIONE FINALE AUTOML (CASCATA): {dominio}\n{'='*80}")
    ...
    if not isinstance(pacchetto, dict):
        raise ValueError(f"Il file {model_path} non è un ecosistema a cascata valido! Ricevuto tipo: {type(pacchetto)}")
    ...
    if metadata:
        print(f"Modelli AutoML ricaricati: L1={metadata.get('Triage_L1')}, L2={metadata.get('Oracolo_L2')}, Soglia={soglia}")
    ...
    # 1. Chiediamo al classificatore L1 di testare confidenza
    ...
    # 2. Routing dinamico (Se > Soglia usa L1, altrimenti L2)
    ...
    print(f"📊 Utilizzo Modelli: Triage (L1) ha risolto {uso_albero} istanze, Oracolo (L2) ne ha valutate {uso_xgboost}.")
    ...
    nome_csv = f"validazione_finale_automl_{dominio}.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Esegue la validazione dei modelli AutoML a cascata pre-estratti dai .joblib")
    ...
    print("\n" + "="*80)
    print(f"CONFRONTO {args.dominio} A CASCATA AUTOML COMPLETATO")