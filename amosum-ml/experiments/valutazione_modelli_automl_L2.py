...
def validazione_finale(dominio, model_path, test_path, original_paths):
    ...
    # Esegue solo l'oracolo L2
    scelte_oracle = oracle.predict(X)
    pred_numeriche = scelte_oracle
    ...
    print(f"📊 Utilizzo Modelli: SOLO Oracolo (L2) ha risolto tutte le {len(X)} istanze.")
    ...
    nome_csv = f"validazione_finale_automl_L2_{dominio}.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Validazione SOLO Modello Oracolo L2")
